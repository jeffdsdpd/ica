...
public class TeacherDao {
	...
	public List<TeacherInteraction> getInteractionTeacherListBySchool(String schoolId) {
		...
		/* From the original ICA Application
		...
		*/
		...
		String sql = "SELECT HOKE_RUBRIC.ID as ID, TEACHERS.NAME, HOKE_RUBRIC.DATE, 'Rubric' AS INTERACTIONMETHOD, HOKE_RUBRIC.USERID, HOKE_RUBRIC.PHASE"
				+ " FROM HOKE_RUBRIC, TEACHERS"
				+ " WHERE HOKE_RUBRIC.TEACHERID = TEACHERS.ID"
				+ " AND HOKE_RUBRIC.SCHOOLID = ?"
				
				+ " UNION"
				
				+ " SELECT HOKE_MODEL_TEACHER_RUBRIC.ID as ID, TEACHERS.NAME, HOKE_MODEL_TEACHER_RUBRIC.DATE, 'Model Teacher' AS INTERACTIONMETHOD, HOKE_MODEL_TEACHER_RUBRIC.USERID, '' "
				+ " FROM HOKE_MODEL_TEACHER_RUBRIC, TEACHERS"
				+ " WHERE HOKE_MODEL_TEACHER_RUBRIC.TEACHERID = TEACHERS.ID"
				+ " AND HOKE_MODEL_TEACHER_RUBRIC.SCHOOLID = ?"
				
				+ " UNION"
				
				+ " SELECT COACHING_INTERACTIONS.ID as ID, TEACHERS.NAME, COACHING_INTERACTIONS.DATE, 'Coaching' AS INTERACTIONMETHOD, COACHING_INTERACTIONS.USERID, '' "
				+ " FROM COACHING_INTERACTIONS, TEACHERS"
				+ " WHERE COACHING_INTERACTIONS.TEACHERID = TEACHERS.ID"
				+ " AND COACHING_INTERACTIONS.SCHOOLID = ?"
				+ " ORDER BY DATE DESC";
